...
<template>
  ...
  ══════════════════════════════════════════════════════════ -->
  <div class="flex min-h-screen transition-colors duration-500 font-sans"
    :class="isDark ? 'bg-[#1e2a3a] text-slate-200' : 'bg-slate-100 text-slate-800'">
    ...
    <Transition name="notification">
      <div v-if="notification.show"
        ...
        <div class="w-7 h-7 rounded-xl flex items-center justify-center shrink-0"
          :class="notification.type === 'success' ? 'bg-emerald-500/20' : 'bg-rose-500/20'">
          <i :class="notification.type === 'success' ? 'fas fa-check text-[10px]' : 'fas fa-xmark text-[10px]'"></i>
        </div>
        <p class="text-[11px] font-bold tracking-wide">{{ notification.message }}</p>
      </div>
    </Transition>
    ...
    <aside :class="[
      ...
      isDark ? 'bg-[#162030] border-white/[0.06]' : 'bg-white border-slate-200',
      isSidebarOpen ? 'w-60 translate-x-0' : 'w-0 -translate-x-full lg:w-[62px] lg:translate-x-0'
    ]">
      ...
      <button @click="isSidebarOpen = !isSidebarOpen"
        ...
        :class="[
          isDark ? 'border-white/10 bg-[#162030] text-white/40' : 'border-slate-200 bg-white text-slate-400',
      ...
      <div class="flex flex-col h-full overflow-hidden">
        ...
        <div class="px-3 pt-5 pb-6 flex items-center gap-3" :class="!isSidebarOpen && 'lg:justify-center lg:px-0'">
          <div class="w-9 h-9 shrink-0 rounded-xl flex items-center justify-center text-white font-black text-sm shadow-lg shadow-[#3B82F6]/30"
          ...
          <div v-if="isSidebarOpen" class="overflow-hidden">
            ...
            <p class="text-[9px] font-medium tracking-widest uppercase mt-0.5"
              :class="isDark ? 'text-white/30' : 'text-slate-400'">Control Panel</p>
          </div>
        </div>
        ...
        <nav class="flex-1 px-2 space-y-0.5 overflow-y-auto overflow-x-hidden">
          <template v-for="(item, key) in {
            ...
            <button @click="currentTab = key"
              :title="item.label"
              class="group w-full flex items-center gap-3 rounded-xl transition-all duration-150 relative"
              :class="[
                isSidebarOpen ? 'px-2.5 py-2' : 'lg:justify-center lg:px-0 lg:py-2.5',
                currentTab === key
              ...
              <div class="w-7 h-7 shrink-0 rounded-lg flex items-center justify-center transition-all text-[12px]"
                :class="currentTab === key ? 'bg-[#3B82F6]/20 text-[#3B82F6]' : [item.bg, item.color]">
                <i :class="item.icon"></i>
              </div>
        ...
        <div v-if="employee?.isSuperAdmin" class="px-2 pb-2 space-y-0.5">
          <p v-if="isSidebarOpen" class="px-2 text-[8px] font-black uppercase tracking-widest mb-1"
            :class="isDark ? 'text-white/20' : 'text-slate-300'">Dev</p>
          ...
          <button v-for="devItem in [
            ...
            @click="router.push(devItem.path)" :title="devItem.label"
        ...
        <!-- Footer: tema + logout -->
        <div class="px-2 pb-4 space-y-0.5 pt-3 border-t"
          :class="isDark ? 'border-white/5' : 'border-slate-100'">
          <button @click="toggleTheme"
            class="w-full flex items-center gap-2.5 px-2 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all"
            :class="[
              isDark ? 'text-white/30 hover:text-white/70 hover:bg-white/5' : 'text-slate-400 hover:text-slate-700 hover:bg-slate-100',
              !isSidebarOpen && 'lg:justify-center'
            ]">
            <i :class="isDark ? 'fas fa-sun text-yellow-400' : 'fas fa-moon text-indigo-400'" class="text-[11px]"></i>
    ...
    <main class="flex-1 flex flex-col h-screen overflow-hidden min-w-0">
      ...
      <header class="h-12 shrink-0 flex items-center justify-between px-6 border-b backdrop-blur-sm"
        :class="isDark
        ...
        <div class="flex items-center gap-3">
          <div class="hidden sm:flex items-center gap-1.5 px-3 py-1 rounded-full border"
            :class="isDark ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-emerald-300 bg-emerald-50'">
          ...
          <div v-if="employee?.name"
            class="flex items-center gap-2 px-2.5 py-1 rounded-xl border cursor-default"
      ...
      <!-- Contenido -->
      <div class="flex-1 overflow-y-auto p-4 lg:p-5 flex flex-col"
        :class="isDark ? 'bg-[#1e2a3a]' : 'bg-slate-100'">
        ...
        <div v-if="currentTab === 'companies'" class="animate-fade-in">
          <GestionCompanias :isDark="isDark" @success="showNotification($event)" @error="showNotification($event, 'error')" />
        </div>

        <div v-if="currentTab === 'users'" class="animate-fade-in">
          <GestionUsuarios :isDark="isDark" @success="showNotification($event)" @error="showNotification($event, 'error')" @open-perms="openPerms($event)" />
        </div>

        <div v-if="currentTab === 'notifications'" class="animate-fade-in">
          <Notificaciones :isDark="isDark" :apiUrl="API_URL" @notification-sent="showNotification('Notificación enviada')" />
        </div>

        <div v-if="currentTab === 'estructura'" class="animate-fade-in">
        ...
        <div v-if="currentTab === 'config'" class="animate-fade-in">
          <GestionConfiguraciones :isDark="isDark" @success="showNotification($event)" @error="showNotification($event, 'error')" />
        </div>

        <div v-if="currentTab === 'api'" class="animate-fade-in">
          <GestionApiExterna :isDark="isDark" @success="showNotification($event)" @error="showNotification($event, 'error')" />
        </div>
