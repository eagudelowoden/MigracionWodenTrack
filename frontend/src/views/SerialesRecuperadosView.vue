<template>
  <div
    class="vercel-root min-h-screen w-screen flex flex-col items-center transition-colors duration-200"
    :class="isDark ? 'dark' : ''"
  >
    <div class="w-full max-w-3xl px-5 py-6 space-y-6">
      ...
      <header class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div class="flex items-center gap-2">
            <div class="v-logo">
              <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2L2 19.5h20L12 2z"/></svg>
            </div>
            <div class="leading-tight">
              <h1 class="text-[15px] font-semibold tracking-tight v-fg">Seriales Recuperados</h1>
              <p class="text-[11px] v-muted">Woden FSM · Consulta</p>
            </div>
          </div>
        </div>
        <button @click="toggleTheme" class="v-icon-btn" aria-label="Tema">
          <svg v-if="isDark" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="5"/><path stroke-linecap="round" d="M12 1v2M12 21v2M4.2 4.2l1.4 1.4M18.4 18.4l1.4 1.4M1 12h2M21 12h2M4.2 19.8l1.4-1.4M18.4 5.6l1.4-1.4"/></svg>
          <svg v-else width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z"/></svg>
        </button>
      </header>
      ...
      <section class="v-card p-4">
        <div class="grid grid-cols-1 sm:grid-cols-[1fr_1fr_auto] gap-3 sm:items-end">
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <input type="text" v-model="filtros.documento" placeholder="Ej. 1035851539" @keyup.enter="consultar" class="v-input" />
          </div>
          <button @click="consultar" :disabled="loading" class="v-btn-primary">
            <span v-if="loading" class="v-spinner"></span>
            <template v-else>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="7"/><path stroke-linecap="round" d="M21 21l-4.3-4.3"/></svg>
              Consultar
            </template>
          </button>
        </div>
      ...
      <transition name="fade">
        <div v-if="error" class="v-alert">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path stroke-linecap="round" d="M12 8v4M12 16h.01"/></svg>
          <span>{{ error }}</span>
        </div>
      </transition>
      ...
      <section v-if="registros !== null && !loading" class="space-y-3">
        ...
        <div v-if="totalFiltrado === 0" class="v-card v-empty">
          <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>
          ...
          <article v-for="(item, idx) in visibles" :key="idx" class="v-card v-row">
            ...
            <div class="flex items-center justify-between gap-3 mb-3">
              <div class="flex items-center gap-2.5 min-w-0">
                ...
                <code class="text-[12.5px] font-medium v-fg truncate">{{ item.serial || item.serial_confirmado || '—' }}</code>
              </div>
              <a v-if="linkComprobante(item)" :href="linkComprobante(item)" target="_blank" rel="noopener" class="v-link-btn shrink-0">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3"/></svg>
                Comprobante
              </a>
            </div>
            <!-- Detalle -->
      ...
      <section v-if="registros === null && !loading && !error" class="v-card v-empty">
        <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" d="M4 5v14M8 5v14M12 5v14M16 5v14M20 5v14"/></svg>
...
.v-icon-btn:hover { border-color: var(--border-strong); background: var(--hover); }
.v-icon-btn:active { transform: scale(.95); }
...
.v-row { padding: 14px 16px; transition: border-color .15s ease; }
.v-row:hover { border-color: var(--border-strong); }

/* Labels e inputs */
.v-label { font-size: 12px; font-weight: 500; color: var(--fg); }
.v-input {
...
.dark .v-input { color-scheme: dark; }
.v-input::placeholder { color: var(--muted); }
.v-input:focus { border-color: var(--fg); box-shadow: 0 0 0 1px var(--fg); }
...
.v-btn-primary:hover:not(:disabled) { opacity: .85; }
...
.v-btn-ghost:hover { background: var(--hover); border-color: var(--border-strong); }
...
.v-search {
  ...
  width: 100%; max-width: 260px; transition: border-color .15s ease;
}
.v-search:focus-within { border-color: var(--fg); }
.v-search input {
...
.v-link-btn {
  ...
  border: 1px solid var(--border); background: var(--card); color: var(--fg);
  font-size: 11.5px; font-weight: 500; transition: all .15s ease;
}
.v-link-btn:hover { background: var(--hover); border-color: var(--border-strong); }

/* Definiciones */
.v-dt { font-size: 10.5px; color: var(--muted); margin-bottom: 1px; letter-spacing: .01em; }
.v-dd { font-size: 12.5px; color: var(--fg); font-weight: 450; }

/* Badge estatus */
.v-badge {
  display: inline-flex; align-items: center; gap: 5px;
  height: 22px; padding: 0 9px 0 8px; border-radius: 6px;
  font-size: 11px; font-weight: 500; white-space: nowrap;
  border: 1px solid var(--border); background: var(--bg); color: var(--fg);
}
.v-dot { width: 6px; height: 6px; border-radius: 50%; background: var(--muted); }
.v-badge.is-ok .v-dot { background: #10b981; }
.v-badge.is-warn .v-dot { background: #f59e0b; }
.v-badge.is-err .v-dot { background: #ef4444; }
.v-badge.is-neutral .v-dot { background: #8f8f8f; }
