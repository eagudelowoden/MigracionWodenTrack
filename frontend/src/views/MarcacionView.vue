    ...
    <footer class="w-full max-w-[450px] space-y-3">
      ...
      <button v-if="canRegistrarNovedad" @click="showChoiceModal = true"
        ...
        <div class="flex items-center gap-2">
          <div class="w-6 h-6 rounded-lg flex items-center justify-center transition-all duration-300 group-hover:bg-[#ff8f00] group-hover:text-black"
        ...
        <i class="fas fa-chevron-right text-[8px] opacity-20 group-hover:opacity-60 group-hover:text-[#ff8f00] transition-all group-hover:translate-x-0.5"></i>
      </button>
        ...
        <div class="w-full max-w-sm rounded-2xl border shadow-2xl overflow-hidden"
          ...
          <div class="p-4 flex flex-col gap-3">
            ...
            <button @click="goIndividual"
              ...
              <div class="w-10 h-10 rounded-xl bg-[#ff8f00] flex items-center justify-center shrink-0 shadow-md shadow-orange-500/30 group-hover:scale-105 transition-transform">
        ...
        <div class="w-full max-w-2xl rounded-2xl border shadow-2xl flex flex-col max-h-[90vh]"
          ...
          <div class="flex-1 overflow-y-auto p-5 flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-2">
              ...
              <div v-else class="max-h-48 overflow-y-auto rounded-xl border divide-y"
                ...
                <div v-if="!filteredEmployees.length" class="px-4 py-6 text-center text-[10px] font-bold uppercase opacity-40"
                  :class="isDark ? 'text-slate-400' : 'text-slate-500'">
                  Sin empleados disponibles
                </div>

                <label v-for="emp in filteredEmployees" :key="emp.cc"
                  class="flex items-center gap-3 px-3 py-2 cursor-pointer transition-all"
                  :class="isSelected(emp)
            ...
            <transition name="fade-msg">
              <div v-if="cStatus.msg" class="flex items-center gap-2 px-3 py-2.5 rounded-lg text-[11px] font-bold border"
                :class="cStatus.type === 'ok'
                  ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20'
                  : cStatus.type === 'error'
                    ? 'bg-red-500/10 text-red-400 border-red-500/20'
                    : 'bg-amber-500/10 text-amber-400 border-amber-500/20'">
                <i :class="cStatus.type === 'ok' ? 'fas fa-check-circle' : cStatus.type === 'error' ? 'fas fa-exclamation-circle' : 'fas fa-circle-notch fa-spin'"></i>
                {{ cStatus.msg }}
              </div>
            </transition>
          ...
          <div class="px-5 py-4 border-t shrink-0 flex items-center justify-between gap-3"
            ...
            <button @click="submitColectiva" :disabled="submittingColectiva || !canSubmitColectiva"
              ...
              {{ submittingColectiva ? `Enviando ${cProgress.current}/${cProgress.total}...` : `Aplicar a ${selectedEmployees.length} empleado${selectedEmployees.length !== 1 ? 's' : ''}` }}
            </button>
          </div>
