<template>
  <div class="border rounded p-4 flex flex-col gap-3"
    ...
    <p class="text-[11px] font-semibold uppercase tracking-wide" :class="isDark ? 'text-[#4a5568]' : 'text-[#6b7280]'">
    ...
    <div class="flex flex-col gap-2.5">
      ...
      <div class="flex items-center gap-3">
        <label class="text-[11px] w-24 shrink-0" :class="isDark ? 'text-[#7a8aa0]' : 'text-[#374151]'">Fecha
          inicio</label>
        <input type="date" :value="fechaInicio" @input="$emit('update:fechaInicio', $event.target.value)"
          class="text-[11px] px-2 py-1 border rounded outline-none transition-colors" :class="isDark
            ? 'bg-[#161B26] border-[#222938] text-[#E2E8F0] focus:border-[#3B82F6]'
            : 'bg-white border-[#d1d5db] text-[#111] focus:border-[#3B82F6]'" />
        <span v-if="advertenciaFechaInicio" class="text-[10px] font-medium px-2 py-0.5 rounded"
      ...
      <!-- Fecha fin -->
      <div class="flex items-center gap-3">
        <label class="text-[11px] w-24 shrink-0" :class="isDark ? 'text-[#7a8aa0]' : 'text-[#374151]'">Fecha fin</label>
        <input type="date" :value="fechaFin" :disabled="indefinidamente"
          ...
          :class="isDark
            ? 'bg-[#161B26] border-[#222938] text-[#E2E8F0] focus:border-[#3B82F6]'
            : 'bg-white border-[#d1d5db] text-[#111] focus:border-[#3B82F6]'" />
        <span v-if="!indefinidamente && advertenciaFechaFin" class="text-[10px] font-medium px-2 py-0.5 rounded"
          :class="isDark ? 'bg-yellow-900/40 text-yellow-400' : 'bg-yellow-50 text-yellow-700'">
      ...
      <label class="flex items-center gap-2 cursor-pointer w-fit">
        ...
        <span class="text-[11px]" :class="isDark ? 'text-[#7a8aa0]' : 'text-[#374151]'">Sin fecha de vencimiento
          (indefinidamente)</span>
      </label>
    </div>

    <!-- Nota tipo de asignación -->
    <div class="text-[10px] leading-relaxed mt-1 px-2 py-1.5 rounded" :class="indefinidamente || !fechaFin
      ...
      <i class="fas mr-1 text-[9px]" :class="indefinidamente || !fechaFin ? 'fa-infinity' : 'fa-clock'"></i>
