  ...
  <div class="h-full animate-in fade-in duration-500 flex flex-col gap-2 p-3 overflow-hidden">
      ...
      <div class="flex items-center gap-2">
        ...
        <div>
          ...
          <p class="text-[10px] leading-tight" :class="isDark ? 'text-slate-500' : 'text-slate-400'">
            <code>employee_id</code> directo, sin emparejar turnos ni cruzar mallas/cédulas.
          </p>
        </div>
      </div>
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
        <div class="px-3 py-1.5 rounded-md border flex items-center justify-between gap-2" :class="isDark ? 'bg-[#161B26] border-[#222938]' : 'bg-white border-slate-200'">
          <span class="text-[10px] uppercase tracking-wide font-medium" :class="isDark ? 'text-slate-500' : 'text-slate-400'">Asistencias</span>
          <span class="text-[13px] font-bold tabular-nums" :class="isDark ? 'text-white' : 'text-slate-900'">{{ crudoData.attendancesCount }}</span>
        </div>
        <div class="px-3 py-1.5 rounded-md border flex items-center justify-between gap-2" :class="isDark ? 'bg-[#161B26] border-[#222938]' : 'bg-white border-slate-200'">
          <span class="text-[10px] uppercase tracking-wide font-medium" :class="isDark ? 'text-slate-500' : 'text-slate-400'">Logs biométrico/app</span>
          <span class="text-[13px] font-bold tabular-nums" :class="isDark ? 'text-white' : 'text-slate-900'">{{ crudoData.logsCount }}</span>
        </div>
        <div class="px-3 py-1.5 rounded-md border flex items-center justify-between gap-2" :class="isDark ? 'bg-[#161B26] border-[#222938]' : 'bg-white border-slate-200'">
          <span class="text-[10px] uppercase tracking-wide font-medium" :class="isDark ? 'text-slate-500' : 'text-slate-400'">Tiempo total</span>
          <span class="text-[13px] font-bold tabular-nums" :class="isDark ? 'text-white' : 'text-slate-900'">{{ (crudoData.tiempoTotalMs / 1000).toFixed(1) }}s</span>
        </div>
        <div class="px-3 py-1.5 rounded-md border flex items-center justify-between gap-2" :class="isDark ? 'bg-[#161B26] border-[#222938]' : 'bg-white border-slate-200'">
          <span class="text-[10px] uppercase tracking-wide font-medium" :class="isDark ? 'text-slate-500' : 'text-slate-400'">Heap tras descarga</span>
          <span class="text-[13px] font-bold tabular-nums whitespace-nowrap" :class="isDark ? 'text-white' : 'text-slate-900'">
            {{ crudoData.memoria?.heapMb_tras_descargarLogs ?? '—' }} MB
            <span class="text-[9px] font-normal opacity-60">({{ crudoData.memoria?.heapMb_inicio }} MB inicio)</span>
          </span>
        </div>
      </div>
