  ...
  <div class="h-full flex flex-col animate-fade-in" :class="isDark ? 'text-slate-100' : 'text-slate-800'">
    ...
    <div class="flex items-center justify-between px-4 py-2.5 border-b shrink-0"
      ...
      <div class="flex items-center gap-2">
        <div class="w-7 h-7 rounded-lg flex items-center justify-center shrink-0"
          ...
          <i class="fas fa-sliders text-orange-500 text-[10px]"></i>
        </div>
        <div>
          <h1 class="text-xs font-bold leading-tight">Configuración del Sistema</h1>
          <p class="text-[10px] mt-0.5" :class="isDark ? 'text-white/40' : 'text-slate-400'">Versión {{ appVersion }}</p>
        </div>
      </div>
      <div class="flex items-center gap-2">
        <div class="flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[10px] font-medium"
        ...
        <button @click="guardar" :disabled="saving"
          class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-semibold transition-all active:scale-95 disabled:opacity-50"
          style="background:linear-gradient(135deg,#f97316,#ea580c);color:#fff">
          <i :class="saving ? 'fas fa-spinner fa-spin text-[9px]' : 'fas fa-floppy-disk text-[9px]'"></i>
    ...
    <div class="flex border-b shrink-0"
      ...
      <button v-for="tab in tabs" :key="tab.id" @click="activeTab = tab.id"
        class="flex items-center gap-1.5 px-4 py-2 text-[10px] font-semibold border-b-2 -mb-px transition-all"
    ...
    <div class="flex-1 min-h-0 overflow-y-auto custom-scrollbar">
      ...
      <!-- ═══ TAB SISTEMA ═══════════════════════════════════════════════════ -->
      <div v-if="activeTab === 'sistema'" class="p-3 space-y-3">

        <!-- Mantenimiento -->
        <div class="rounded-xl overflow-hidden transition-all"
          :class="mantenimiento.enabled
            ? isDark ? 'bg-rose-950/30' : 'bg-rose-50'
            : isDark ? 'bg-white/3' : 'bg-white'">

          <div class="px-3.5 py-2.5 border-b flex items-center justify-between gap-3"
            :class="mantenimiento.enabled
              ? isDark ? 'border-rose-500/20' : 'border-rose-200'
              : isDark ? 'border-white/8' : 'border-slate-200'">
            <div class="flex items-center gap-2">
              <div class="w-7 h-7 rounded-lg flex items-center justify-center shrink-0"
                :class="mantenimiento.enabled ? 'bg-rose-500/20' : isDark ? 'bg-white/5' : 'bg-slate-100'">
                <i class="fas fa-hard-hat text-[10px]"
                  :class="mantenimiento.enabled ? 'text-rose-400' : isDark ? 'text-white/40' : 'text-slate-500'"></i>
              </div>
              <div>
                <p class="text-[11px] font-semibold" :class="mantenimiento.enabled ? isDark ? 'text-rose-300' : 'text-rose-700' : ''">
                  Modo mantenimiento IIS
                </p>
                <p class="text-[10px] mt-0.5" :class="isDark ? 'text-white/40' : 'text-slate-500'">
            ...
            <button @click="toggleMantenimiento" :disabled="mantenimiento.saving || !mantenimiento.configured"
              ...
              <span v-if="mantenimiento.saving" class="absolute inset-0 flex items-center justify-center">
                <i class="fas fa-spinner fa-spin text-white text-[9px]"></i>
              </span>
          ...
          <div class="px-3.5 py-2.5" :class="isDark ? '' : ''">
            <div v-if="!mantenimiento.configured" class="flex items-center gap-2 text-[10px] text-amber-500">
              <i class="fas fa-triangle-exclamation shrink-0"></i>
              <span>La variable <code class="font-mono bg-amber-500/10 px-1 py-0.5 rounded text-[9px]">WEBCONFIG_PATH</code> no está configurada en el servidor.</span>
            </div>
            <div v-else class="flex items-center justify-between gap-3">
              ...
              <span class="flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-medium border shrink-0"
                :class="mantenimiento.enabled
        ...
        <!-- Almacenamiento -->
        <div class="rounded-xl overflow-hidden" :class="isDark ? 'bg-white/3' : 'bg-white'">
          <div class="px-3.5 py-2.5 border-b flex items-center gap-2"
            ...
            <div>
              <p class="text-[11px] font-semibold">Almacenamiento de soportes</p>
              <p class="text-[10px] mt-0.5" :class="isDark ? 'text-white/40' : 'text-slate-500'">
          ...
          <div class="px-3 py-3 flex gap-2">
            <button v-for="opt in storageOpts" :key="opt.v" @click="config.storage_mode = opt.v"
              class="flex-1 flex items-center gap-2 p-3 rounded-xl border-2 transition-all text-left"
              :class="config.storage_mode === opt.v
                ? 'border-orange-500 bg-orange-500/10'
                : isDark ? 'border-white/8 hover:border-white/20' : 'border-slate-200 hover:border-slate-300'">
              <i :class="[opt.icon, 'text-base', config.storage_mode === opt.v ? 'text-orange-400' : isDark ? 'text-white/30' : 'text-slate-400']"></i>
              <div class="flex-1 min-w-0">
                <p class="text-[11px] font-semibold"
                  :class="config.storage_mode === opt.v ? 'text-orange-400' : isDark ? 'text-white/80' : 'text-slate-700'">
                  {{ opt.label }}
                </p>
                <p class="text-[10px] mt-0.5" :class="isDark ? 'text-white/35' : 'text-slate-500'">{{ opt.desc }}</p>
              </div>
              <i v-if="config.storage_mode === opt.v" class="fas fa-check-circle text-orange-400 text-[10px] shrink-0"></i>
            </button>
          </div>
        </div>
      </div>
      ...
      <!-- ═══ TAB MÓDULOS ══════════════════════════════════════════════════ -->
      <div v-if="activeTab === 'modulos'" class="p-3 space-y-2">
        <p class="text-[10px] px-1 pb-1" :class="isDark ? 'text-white/40' : 'text-slate-500'">
          Los módulos inactivos muestran un mensaje de mantenimiento a los usuarios en lugar del contenido.
        </p>
        <div v-for="mod in modulos" :key="mod.key" class="rounded-xl overflow-hidden transition-all"
          :class="isActive(mod.key)
            ? isDark ? 'bg-white/3' : 'bg-white'
            : isDark ? 'bg-rose-950/20' : 'bg-rose-50'">
          ...
          <div class="px-3.5 py-2.5 flex items-center gap-3">
            ...
            <div class="flex-1 min-w-0">
              <p class="text-[11px] font-semibold">{{ mod.label }}</p>
              <p class="text-[10px] mt-0.5" :class="isDark ? 'text-white/40' : 'text-slate-500'">{{ mod.desc }}</p>
            </div>
            <div class="flex items-center gap-2 shrink-0">
              ...
              <button @click="toggleModulo(mod.key)"
                class="relative w-10 h-5 rounded-full transition-all duration-300 shrink-0 focus:outline-none"
          ...
          <Transition name="slide">
            <div v-if="!isActive(mod.key)" class="px-3.5 pb-2.5">
              <div class="flex items-center gap-2 px-2.5 py-2 rounded-lg border"
                :class="isDark ? 'border-white/8 bg-white/3' : 'border-slate-100 bg-slate-50'">
                <i class="fas fa-message text-[9px]" :class="isDark ? 'text-white/25' : 'text-slate-400'"></i>
      ...
      <!-- ═══ TAB PROGRAMACIÓN ══════════════════════════════════════════════ -->
      <div v-if="activeTab === 'schedule'" class="p-3 space-y-3">
        ...
        <div class="rounded-xl overflow-hidden" :class="isDark ? 'bg-white/3' : 'bg-white'">
          <div class="px-3.5 py-2.5 flex items-center gap-3">
            ...
            <div class="flex-1">
              <p class="text-[11px] font-semibold">Control de fechas de cargue</p>
              <p class="text-[10px] mt-0.5" :class="isDark ? 'text-white/40' : 'text-slate-500'">
            ...
            <button
              ...
              <span class="absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all duration-300"
                :class="config.mallas_schedule_enabled === 'true' ? 'left-[22px]' : 'left-0.5'"></span>
            </button>
          </div>
        </div>
        ...
        <div v-if="config.mallas_schedule_enabled !== 'true'"
          ...
          <i class="fas fa-unlock text-emerald-500 text-lg shrink-0"></i>
          <div>
            <p class="text-[11px] font-semibold" :class="isDark ? 'text-emerald-400' : 'text-emerald-700'">
              Cargue siempre disponible
            </p>
            <p class="text-[10px] mt-0.5" :class="isDark ? 'text-emerald-500/60' : 'text-emerald-600/70'">
        ...
        <template v-if="config.mallas_schedule_enabled === 'true'">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <button v-for="m in SCHEDULE_MODES" :key="m.value" @click="config.mallas_schedule_mode = m.value"
              ...
              :class="config.mallas_schedule_mode === m.value
                ...
                : isDark ? 'border-white/8 hover:border-white/20' : 'border-slate-200 hover:border-slate-300'">
              <i :class="[m.icon, 'text-lg', config.mallas_schedule_mode === m.value ? 'text-indigo-400' : isDark ? 'text-white/25' : 'text-slate-400']"></i>
          ...
          <div v-if="config.mallas_schedule_mode === 'weekly'"
            ...
            :class="isDark ? 'bg-white/3' : 'bg-white'">
            <div class="px-3.5 py-2 border-b"
              :class="isDark ? 'border-white/8' : 'border-slate-200'">
              <p class="text-[10px] font-semibold" :class="isDark ? 'text-white/60' : 'text-slate-600'">
            ...
            <div class="p-3 grid grid-cols-7 gap-1.5">
              <button v-for="d in diasSemanaFull" :key="d.v"
                ...
                class="flex flex-col items-center gap-0.5 py-2 px-0.5 rounded-lg border-2 transition-all"
                ...
                <span v-if="d.isToday"
                  class="text-[7px] font-black px-1 py-0.5 rounded-full bg-indigo-500/20 text-indigo-400">
                  Hoy
                </span>
              </button>
            </div>
            <div v-if="scheduleWeeklyDays.length" class="px-3.5 pb-2.5">
              <p class="text-[10px]" :class="isDark ? 'text-white/40' : 'text-slate-500'">
                Habilitado cada
          ...
          <div v-if="config.mallas_schedule_mode === 'monthly'"
            class="rounded-xl overflow-hidden"
            :class="isDark ? 'bg-white/3' : 'bg-white'">
            <div class="px-3.5 py-2 border-b flex items-center justify-between"
              :class="isDark ? 'border-white/8' : 'border-slate-200'">
              ...
              <p class="text-[11px] font-semibold capitalize">{{ monthName }} {{ calYear }}</p>
            ...
            <div class="p-3">
              ...
              <div class="grid grid-cols-7 mb-1.5">
                <div v-for="h in ['Lu','Ma','Mi','Ju','Vi','Sá','Do']" :key="h"
                  class="text-center text-[9px] font-black uppercase py-1"
              ...
              <div class="grid grid-cols-7 gap-0.5">
                <div v-for="(cell, idx) in calendarGrid" :key="idx">
                  <button v-if="cell" @click="toggleScheduleDay('monthly', cell)"
                    class="w-full aspect-square rounded-lg text-[11px] font-semibold transition-all flex items-center justify-center"
              ...
              <p v-if="scheduleMonthlyDays.length" class="mt-2 text-[10px] text-center"
          ...
          <div v-if="config.mallas_schedule_mode === 'specific'"
            ...
            <div class="p-3">
              ...
              <div v-if="scheduleSpecificDates.length" class="mt-3 border-t pt-2.5 flex flex-wrap gap-1"
              ...
              <p v-else class="mt-2 text-[10px] text-center opacity-40">
          ...
          <!-- Próximas fechas -->
          <div v-if="proximasFechas.length" class="rounded-xl p-3"
            :class="isDark ? 'bg-indigo-500/5' : 'bg-indigo-50'">
            <p class="text-[10px] font-semibold uppercase tracking-wide mb-2"
              :class="isDark ? 'text-indigo-400' : 'text-indigo-600'">
              <i class="fas fa-calendar-circle-user mr-1"></i>Próximas ventanas habilitadas
            </p>
          ...
          <div v-else-if="config.mallas_schedule_mode !== 'free'"
            class="flex items-center gap-2.5 p-3 rounded-xl text-[10px]"
            :class="isDark ? 'bg-amber-500/5 text-amber-400' : 'bg-amber-50 text-amber-700'">
            <i class="fas fa-triangle-exclamation shrink-0"></i>
      ...
      <!-- ═══ TAB CORREO ═══════════════════════════════════════════════════════ -->
      <div v-if="activeTab === 'correo'" class="p-3 space-y-3">
        ...
        <div class="rounded-xl overflow-hidden" :class="isDark ? 'bg-white/3' : 'bg-white'">
          <div class="px-3.5 py-2.5 border-b flex items-center justify-between gap-3"
            ...
            <div class="flex items-center gap-2">
              <div class="w-7 h-7 rounded-lg flex items-center justify-center"
                ...
                <i class="fas fa-gear text-blue-500 text-[10px]"></i>
              </div>
              <div>
                <p class="text-[11px] font-bold" :class="isDark ? 'text-white' : 'text-slate-800'">Configuración SMTP</p>
                <p class="text-[9px]" :class="isDark ? 'text-white/40' : 'text-slate-400'">Outlook / Office 365 — smtp.office365.com:587</p>
              </div>
            </div>
            <div class="flex items-center gap-1.5">
              <div class="w-1.5 h-1.5 rounded-full" :class="correo.form.habilitado ? 'bg-emerald-400 animate-pulse' : 'bg-slate-400'"></div>
          ...
          <div class="p-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div class="flex items-end">
              <div class="w-full flex items-center justify-between p-2.5 rounded-lg border"
          ...
          <div class="px-3 pb-3 flex gap-2">
            <button @click="testConexion" :disabled="correo.testeando"
              class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-[10px] font-bold uppercase tracking-wide transition-all disabled:opacity-50"
            ...
            <button @click="guardarConfigCorreo" :disabled="correo.guardando"
              class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-500 text-white text-[10px] font-bold uppercase tracking-wide hover:bg-blue-400 disabled:opacity-50 transition-all">
              <i class="fas text-[9px]" :class="correo.guardando ? 'fa-circle-notch fa-spin' : 'fa-floppy-disk'"></i>
              Guardar SMTP
            </button>
          </div>
          <div v-if="correo.testResult" class="mx-3 mb-3 px-2.5 py-1.5 rounded-lg text-[10px] font-semibold border"
        ...
        <div class="rounded-xl overflow-hidden" :class="isDark ? 'bg-white/3' : 'bg-white'">
          <div class="px-3.5 py-2.5 border-b flex items-center gap-2"
            ...
            <div class="w-7 h-7 rounded-lg flex items-center justify-center"
              ...
              <i class="fas fa-users text-violet-500 text-[10px]"></i>
            </div>
            <div>
              <p class="text-[11px] font-bold" :class="isDark ? 'text-white' : 'text-slate-800'">Destinatarios de Novedades</p>
              <p class="text-[9px]" :class="isDark ? 'text-white/40' : 'text-slate-400'">Recibirán un correo cuando el jefe o RRHH apruebe/rechace una novedad</p>
            </div>
          </div>
          <div class="p-3 space-y-2">
            <!-- Lista de destinatarios -->
            <div v-if="correo.destinatarios.length" class="flex flex-wrap gap-1.5">
              <div v-for="email in correo.destinatarios" :key="email"
                class="flex items-center gap-1 px-2 py-1 rounded-full border text-[10px] font-semibold"
                :class="isDark ? 'bg-violet-500/10 border-violet-500/30 text-violet-300' : 'bg-violet-50 border-violet-200 text-violet-700'">
                <i class="fas fa-envelope text-[8px] opacity-60"></i>
                {{ email }}
                <button @click="quitarDestinatario(email)" class="ml-0.5 opacity-50 hover:opacity-100 transition-opacity">
                  <i class="fas fa-xmark text-[8px]"></i>
                </button>
              </div>
            </div>
            <p v-else class="text-[10px] py-2 text-center" :class="isDark ? 'text-white/30' : 'text-slate-400'">
            ...
            <div class="flex gap-1.5">
              <input v-model="correo.nuevoDestinatario" type="email" placeholder="correo@empresa.com"
                ...
                class="flex-1 px-2.5 py-1.5 rounded-lg border text-[10px] outline-none transition-all"
              ...
              <button @click="agregarDestinatario"
                class="px-2.5 py-1.5 rounded-lg border text-[10px] font-bold transition-all"
              ...
              <button @click="guardarDestinatarios" :disabled="correo.guardandoDest"
                class="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-violet-500 text-white text-[10px] font-bold uppercase tracking-wide hover:bg-violet-400 disabled:opacity-50 transition-all">
        ...
        <div class="rounded-xl overflow-hidden" :class="isDark ? 'bg-white/3' : 'bg-white'">
          <div class="px-3.5 py-2.5 border-b flex items-center gap-2"
            ...
            <div>
              <p class="text-[11px] font-bold" :class="isDark ? 'text-white' : 'text-slate-800'">Prueba de envío</p>
              <p class="text-[9px]" :class="isDark ? 'text-white/40' : 'text-slate-400'">Envía un correo de prueba para verificar que la configuración funciona</p>
            </div>
          </div>
          <div class="p-3 flex gap-1.5">
            ...
            <button @click="enviarCorreoTest" :disabled="correo.enviandoTest || !correo.correoTest"
              class="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-500 text-white text-[10px] font-bold uppercase tracking-wide hover:bg-emerald-400 disabled:opacity-50 transition-all">
          ...
          <div v-if="correo.testEnvioResult" class="mx-3 mb-3 px-2.5 py-1.5 rounded-lg text-[10px] font-semibold border"
