...
export class ReportsService {
  ...
  //     const decimalAHora = (h: number): string => {
  //       const hh = Math.floor(h).toString().padStart(2, '0');
  //       const mm = Math.round((h % 1) * 60)
  //         .toString()
  //         .padStart(2, '0');
  ...
  //       if (nombresEmpleados.length > 0) {
  //         // 1. Buscar cédulas en res.partner
  //         const partners = await this.odoo.executeKw<any[]>(
  //           'res.partner',
  //           'search_read',
  //           [[['name', 'in', nombresEmpleados]]],
  ...
  //           uid,
  //         );
  //         partners?.forEach((p) => cedulaMap.set(p.name, p.doc_number));

  //         // 2. Buscar empleados para obtener su ID y cruzar con contratos
  //         const empleados = await this.odoo.executeKw<any[]>(
  //           'hr.employee',
  //           'search_read',
  //           [[['name', 'in', nombresEmpleados]]],
  ...
  //         const calendarPorNombre = new Map<string, number>(); // nombre -> calendar_id

  //         if (empleados?.length > 0) {
  //           const empleadoIds = empleados.map((e) => e.id);
  //           const nombrePorId = new Map<number, string>(
  //             empleados.map((e) => [e.id, e.name]),
  //           );

  //           // 3. Buscar contratos activos de esos empleados
  //           const contratos = await this.odoo.executeKw<any[]>(
  //             'hr.contract',
  //             'search_read',
  //             [
  //               [
  //                 ['employee_id', 'in', empleadoIds],
  //                 ['state', 'in', ['open', 'draft']],
  //               ],
  //             ],
  //             { fields: ['employee_id', 'job_id', 'resource_calendar_id'] },
  //             uid,
  //           );
  ...
  //           contratos?.forEach((c) => {
  //             const empId: number = c.employee_id?.[0];
  //             const nombreEmp: string = nombrePorId.get(empId) || '';

  //             // Cargo
  //             if (nombreEmp && c.job_id?.[1]) {
  //               cargoMap.set(nombreEmp, c.job_id[1]);
  //             }

  //             // Acumular calendar IDs para consulta de malla
  //             if (c.resource_calendar_id?.[0]) {
  //               calendarIds.push(c.resource_calendar_id[0]);
  //             }
  //           });

  //           // 4. Consultar líneas de malla horaria
  //           if (calendarIds.length > 0) {
  //             const lineasMalla = await this.odoo.executeKw<any[]>(
  //               'resource.calendar.attendance',
  //               'search_read',
  //               [[['calendar_id', 'in', [...new Set(calendarIds)]]]],
  //               {
  ...
  //             // Agrupar por calendar_id
  //             const mallasPorCalendar = new Map<number, any[]>();
  //             lineasMalla?.forEach((l) => {
  //               const cid: number = l.calendar_id[0];
  //               if (!mallasPorCalendar.has(cid)) mallasPorCalendar.set(cid, []);
  //               mallasPorCalendar.get(cid)!.push(l);
  //             });

  //             // Formatear malla: "Lun 06:00-14:00 | Mar 06:00-14:00 ..."
  //             const formatearMalla = (lineas: any[]): string => {
  //               const ordenadas = lineas.sort(
  //                 (a, b) => Number(a.dayofweek) - Number(b.dayofweek),
  //               );
  //               // Agrupar días con mismo horario para compactar
  //               const grupos: { dias: string[]; rango: string }[] = [];
  //               ordenadas.forEach((l) => {
  //                 const rango = `${decimalAHora(l.hour_from)}-${decimalAHora(l.hour_to)}`;
  //                 const dia = DIAS[Number(l.dayofweek)] || `D${l.dayofweek}`;
  //                 const ultimo = grupos[grupos.length - 1];
  //                 if (ultimo && ultimo.rango === rango) {
  ...
  //               });
  //               return grupos
  //                 .map((g) => `${g.dias.join('/')} ${g.rango}`)
  //                 .join(' | ');
  //             };

  //             // Asignar malla por nombre de empleado
  //             contratos?.forEach((c) => {
  //               const empId: number = c.employee_id?.[0];
  //               const nombreEmp: string = nombrePorId.get(empId) || '';
  //               const calId: number = c.resource_calendar_id?.[0];
  //               if (nombreEmp && calId && mallasPorCalendar.has(calId)) {
  //                 mallaMap.set(
  //                   nombreEmp,
  //                   formatearMalla(mallasPorCalendar.get(calId)!),
  //                 );
  //               }
  //             });
  //           }
  ...
  //       }
  //     } catch (odooErr) {
  //       console.error('Error consultando Odoo:', odooErr.message);
  //     }

  //     // 5. Columnas del Excel
  //     worksheet.columns = [
  //       { header: 'COLABORADOR', key: 'colaborador', width: 35 },
  //       { header: 'CÉDULA', key: 'cedula', width: 15 },
  //       { header: 'CARGO', key: 'cargo', width: 30 },
  ...
  //           item.doc_number ||
  //           item.Cédula ||
  //           cedulaMap.get(nombreEmpleado) ||
  //           'N/A',
  //         cargo: item.Cargo || cargoMap.get(nombreEmpleado) || 'N/A',
  //         depto: item.department_id || item.Departamento || 'N/A',
  //         malla: item.Malla || mallaMap.get(nombreEmpleado) || 'N/A',
  //         fecha: item.fecha || item.Fecha || 'N/A',
