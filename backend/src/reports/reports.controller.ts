...
export class ReportsController {
  ...
  async descargarPlantilla(
    ...
    try {
      ...
      const nombreArchivo =
        `plantilla_${companyName}_${departamento || 'general'}`.replace(
          /\s+/g,
          '_',
        );
      ...
      return res
        .status(500)
        .json({ message: 'Error interno al generar el Excel' });
    }
  }
  ...
  async exportarAsistencias(
    @Body() datosFiltrados: any[],
    @Res() res: express.Response,
  ) {
    try {
      // Llamamos al servicio pasando los datos que vienen del frontend
      const buffer =
        await this.reportsService.generarReporteAsistencias(datosFiltrados);
